...
2.  **Synthesize (Dev)**: Use the **DevOps Engineer** with `/implement-dev <spec>` to update the `environments/dev/docker-compose.yml` for testing.
3.  **Synthesize (Prod)**: Use the **DevOps Engineer** with `/implement-prod` to generate the consolidated `environments/prod/docker-compose.yml`.
